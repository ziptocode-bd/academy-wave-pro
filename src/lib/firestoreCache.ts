...
export async function getCachedCollection<T extends { id: string }>(
  ...
  const p = (async () => {
    try {
      ...
      console.warn(`[getCachedCollection] Network/Firestore issue for ${collectionName}:`, err);
      const fallbackLS = getLS<T[]>(cacheKey);
      if (fallbackLS?.data) {
        return fallbackLS.data;
      }
      const fallbackMem = memoryCache.get(cacheKey);
      if (fallbackMem?.data) {
...
export async function getCachedDoc<T>(
  ...
  const p = (async () => {
    try {
      ...
      console.warn(`[getCachedDoc] Network/Firestore issue for ${collectionName}/${docId}:`, err);
      const fallbackLS = getLS<T>(cacheKey);
      if (fallbackLS?.data) return fallbackLS.data;
      const fallbackMem = memoryCache.get(cacheKey);
      if (fallbackMem?.data) return fallbackMem.data as T;
      throw err;
    } finally {
      pendingRequests.delete(cacheKey);
    }
