    ...
    const template = [
      {
        ...
        correct: 1,
        marks: 1,
        explanation: "2 + 2 = 4, because adding two items to two items equals four.",
        explanationImage: ""
      },
      {
      ...
      <div className="px-3 sm:px-4 pt-4 space-y-4">
        ...
        <div className="bg-accent/30 border border-border rounded-xl p-3">
          <p className="text-xs text-muted-foreground">
            Total: <span className="text-foreground font-medium">{questions.length} Questions</span> •
    ...
    <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm">
      ...
      <div className="flex items-center gap-2 px-3 py-2 bg-accent/40 border-b border-border">
        ...
        <span className="text-xs font-medium text-foreground truncate max-w-[200px] sm:max-w-xs">
          {question.questionText ? question.questionText : `Question ${index + 1}`}
        </span>
        ...
        <div className="p-3 space-y-2.5">
          ...
          <div className="pt-2 border-t border-border/60">
            {!showExplanation ? (
              ...
              >
                + ব্যাখ্যা যুক্ত করুন (Add Explanation)
              </button>
            ) : (
              <div className="space-y-2 bg-accent/20 rounded-lg p-2.5 border border-border/50">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-foreground">উত্তরের ব্যাখ্যা (Explanation)</span>
                  <button
                    type="button"
                    onClick={() => {
                      if (!question.explanation && !question.explanationImage) setShowExplanation(false);
                      else {
                        onUpdate({ explanation: "", explanationImage: "" });
                ...
                <textarea
                  ...
                  placeholder="উত্তরের বিস্তারিত ব্যাখ্যা লিখুন..."
                  rows={2}
                  className="w-full px-3 py-2 rounded-lg bg-background border border-border text-foreground text-xs resize-none focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all"
                />
                <ImageUrlField
                  value={question.explanationImage || ""}
                  onChange={(v) => onUpdate({ explanationImage: v })}
                  placeholder="ব্যাখ্যার চিত্র/ছবি URL (ঐচ্ছিক)"
                  small
                />
                {question.explanationImage && (
                  <img src={question.explanationImage} alt="Explanation" className="h-16 rounded-lg object-contain mt-1" />
                )}
              </div>
            )}
          </div>
