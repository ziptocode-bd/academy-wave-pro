...
export default function AdminUsersPage() {
  ...
  const loadPendingData = useCallback(async (silent = false) => {
    ...
    try {
      ...
      // (b1) users with pending account status (single field query to avoid composite index requirements)
      const acctSnap = await getDocs(
        query(collection(db, "users"), where("status", "==", "pending")),
      );
      const acctPendingUsers = acctSnap.docs
      ...
      const pendingEnrollIds = [...new Set(pendingReqs.map(r => r.userId))].filter(
        id => Boolean(id) && !acctPendingIds.has(id),
      );
      ...
      for (const chunk of chunks(pendingEnrollIds, 30)) {
        ...
        try {
          ...
          snap.docs.forEach(d => {
            const u = { id: d.id, ...d.data() } as UserWithId;
            if (u.role !== "admin") enrollPendingUsers.push(u);
          });
        } catch (e) {
          console.warn("Failed to fetch pending user chunk:", e);
        }
      }
  ...
  const loadPagedUsers = useCallback(async (cursor: DocumentSnapshot | null = null) => {
    ...
    try {
      let pageUsers: UserWithId[] = [];
      let newLastDoc: DocumentSnapshot | null = null;
      let more = false;

      try {
        // Try with orderBy createdAt
        const res = await getPaginatedCollection<UserWithId>(
          db,
          "users",
          PAGE_SIZE,
        ...
        console.warn("Retrying users query without orderBy constraint:", err);
        // Fallback without orderBy constraint if createdAt indexing fails
        const res = await getPaginatedCollection<UserWithId>(
          db,
          "users",
          PAGE_SIZE,
      ...
      for (const chunk of chunks(userIds, 30)) {
        ...
        try {
          ...
          console.warn("Failed to load chunk of enroll requests:", e);
        }
      }
