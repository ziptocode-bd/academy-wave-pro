...
export default function AdminExamsPage() {
  ...
  const downloadQuestionsPDF = (exam: Exam) => {
    ...
    exam.questions.forEach((q, idx) => {
      html += `<div class="question"><div class="q-header">Q${idx + 1}. ${q.questionText} <span class="q-type">${q.marks} mark${q.marks > 1 ? "s" : ""}</span></div>`;
      ...
      if (q.explanation || q.explanationImage) {
        html += `<div class="answer-label" style="color:#0284c7;margin-top:10px">Explanation / ব্যাখ্যা:</div>`;
        if (q.explanation) html += `<div style="font-size:13px;color:#334155;background:#f8fafc;padding:8px 12px;border-radius:6px;border-left:3px solid #0284c7;margin-top:4px">${q.explanation}</div>`;
        if (q.explanationImage) html += `<img src="${q.explanationImage}" alt="Explanation" style="max-height:120px;margin-top:6px;border-radius:6px" />`;
      }
      html += `</div>`;
    });
    html += `</body></html>`;
