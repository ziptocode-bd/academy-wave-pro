...
export default function MyCoursesPage() {
  ...
  if (activeChapter !== "All" && activeSubject !== "All" && activeSubject !== "Live") {
    filtered = filtered.filter((v) => v.chapterName === activeChapter);
  }
  filtered = [...filtered].sort((a, b) => Number(!!b.isLive) - Number(!!a.isLive));
    ...
    <div className="fixed inset-x-0 top-14 bottom-14 flex flex-col overflow-hidden sm:static sm:inset-auto sm:top-auto sm:bottom-auto sm:h-full">
      ...
      <div className="sticky top-0 z-10 bg-background px-4 pt-4 pb-2 flex-shrink-0 space-y-2">
        <div className="flex gap-2 overflow-x-auto scrollbar-hide">
          <button
            onClick={() => {
              setActiveSubject("Live");
              setActiveChapter("All");
            }}
            className={`px-3.5 py-1.5 rounded-full text-sm whitespace-nowrap font-semibold shrink-0 flex items-center gap-1.5 transition-all ${
              activeSubject === "Live"
          ...
          >
            <Radio
              className={`h-3.5 w-3.5 ${
                liveVideosCount > 0 ? "text-red-500 animate-pulse" : "text-muted-foreground"
              }`}
            />
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-10 px-4">
            <p className="text-muted-foreground">
              {activeSubject === "Live"
                ? "বর্তমানে কোনো লাইভ ক্লাস চলছে না।"
                : "No videos found."}
            </p>
            {activeSubject === "Live" && (
              <button
            ...
            {filtered.map((video) => {
              return (
                <button
                  ...
                  className={`bg-card rounded-none sm:rounded-xl overflow-hidden border-0 border-b sm:border text-left group hover:shadow-lg transition-all duration-200 w-full active:scale-[0.98] ${
                    video.isLive
                      ? "border-red-500 sm:ring-2 sm:ring-red-500/20"
